...
window.onload = function () {
    ...
    funcId("delButton").addEventListener("click", function () {
        for (let j = 0; j < 10; j++) {
            funcId("numbers").children[j].classList.remove("selected");
        }
        this.classList.add("selected");
...
function makeBoard() {
    ...
    for (let i = 0; i < 81; i++) {
        ...
        funcId("board").children[i].addEventListener("click", async function () {
            ...
            } else if (checkDuplicates(board, parseInt(numToAdd), finalIndexes) == true && numToAdd != undefined && numToAdd != 'del') {
            ...
            } else if (checkDuplicates(board, parseInt(numToAdd), finalIndexes) == false && isSolved == false) {
...
async function solve1() {
    ...
    for (let i = 1; i < 10; i++) {
        if (checkDuplicates(board, i, empty)) {
            ...
            await sleep1();
            funcId("board").children[finalInd].classList.add("solveColour");
            funcId("board").children[finalInd].innerHTML = '';
        }
    }
